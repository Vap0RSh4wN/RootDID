...
async function main(){
    ...
    //Downer_privkey
    const prvKey2 = Buffer.from(Downer_privkey.toString(16),"hex");//其值和bigint直接toString()不同
    console.log('Downer_privkey eddsa.prv2pub(prvKey2): '+F.toString(eddsa.prv2pub(prvKey2)[0]));
    console.log('Downer_privkey eddsa.prv2pub(prvKey2): '+F.toString(eddsa.prv2pub(prvKey2)[1]));
    const signature2 = eddsa.signPoseidon(prvKey2, F.e(BigInt(F.toObject(HashedID))));//F.e(1234)
    console.log("Downer_privkey signature0.R8[0]",F.toObject(signature2.R8[0]));
    console.log("Downer_privkey signature0.R8[1]",F.toObject(signature2.R8[1]));
    console.log("Downer_privkey signature0.S",signature2.S);


    //SIG Owner_privkey
    const prvKey1 = Buffer.from(Owner_privkey.toString(16),"hex");//其值和bigint直接toString()不同
    console.log('SIG_Owner eddsa.prv2pub(prvKey2): '+F.toString(eddsa.prv2pub(prvKey1)[0]));
    console.log('SIG_Owner eddsa.prv2pub(prvKey2): '+F.toString(eddsa.prv2pub(prvKey1)[1]));
    const signature1 = eddsa.signPoseidon(prvKey1, F.e(BigInt(F.toString(eddsa.prv2pub(prvKey2)[0]))+BigInt(F.toString(eddsa.prv2pub(prvKey2)[1]))));//F.e(1234)
    ...
    var DIDleaf = poseidon([DIDu,IDu,salt,F.toString(eddsa.prv2pub(prvKey2)[0]),F.toString(eddsa.prv2pub(prvKey2)[1])]);
