...
PHASE1=../circuits/ptau/powersOfTau28_hez_final_22.ptau
BUILD_DIR=../build/VC_without_predicate
CIRCUIT_NAME=VC_without_predicate
...
start=`date +%s`
node "$BUILD_DIR"/"$CIRCUIT_NAME"_js/generate_witness.js "$BUILD_DIR"/"$CIRCUIT_NAME"_js/"$CIRCUIT_NAME".wasm VC_without_predicate.json "$BUILD_DIR"/witness.wtns
end=`date +%s`
echo "DONE ($((end-start))s)"
